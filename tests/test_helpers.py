...
class HelperTests(TestCase):
    ...
    def test_get_context(self):
        self.assertEqual(
            helpers.get_context("{{a}} {{b.c}}"), {"a": "A", "b": {"c": "C"}}
        )
    ...
    def test_merge_dicts(self):
        self.assertEqual(helpers.merge_dicts({"foo": 1}), {"foo": 1})
        self.assertEqual(
            helpers.merge_dicts({"foo": 1}, {"bar": 2}), {"foo": 1, "bar": 2}
        )
    ...
    def test_patch_context(self):
        ...
        self.assertEqual(helpers.patch_context(foo, []), foo)

        self.assertEqual(
            helpers.patch_context(foo, [cp1]), helpers.merge_dicts(foo, bar)
        )

        self.assertEqual(
            helpers.patch_context(foo, [cp1, cp2]), helpers.merge_dicts(foo, bar, baz)
        )
